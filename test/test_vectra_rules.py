...
def test_create_rule_all_hosts(vc_v2):
    resp = vc_v2.create_rule(
                            detection_category='botnet activity',
                            detection_type='outbound port sweep',
                            triage_category='misconfiguration',
                            description='pytest_all',
                            all_hosts=True,
                            remote1_port=['443']
        )
...
def test_get_rules(vc_v2):
    '''
    Gather rules from brain, gather a specific rule as well
    '''
    resp = vc_v2.get_rules().json()
    assert resp['count'] >= 3

    resp2 = vc_v2.get_rules(name='pytest_hostname')
    assert resp2[0]['description'] == 'pytest_hostname'


def test_update_rule_replace(vc_v2, test_host2):
    host_url = test_host2['url']
    resp = vc_v2.update_rule(
                    rule_id=test_vars['host_rule_id'],
                    host=[host_url],
                    remote1_dns=['foo.com'],
                    remote1_ip=['4.4.4.4'],
                    remote1_port=['8443']
...
def test_update_rule_append(vc_v2, test_host):
    '''
    Test successfully updating rules with IP and DNS addresses
    '''
    resp = vc_v2.update_rule(
