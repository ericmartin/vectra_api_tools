...
def main():
    ...
    parser_create.add_argument('--type',
                        required=True,
                        action='store',
                        choices=['Anonymization', 'C2', 'Exfiltration', 'Malware Artifacts', 'Watchlist'],
    ...
    parser_edit = subparsers.add_parser('update', help='Update STIX file for existing threat feed')
    ...
    parser_delete.add_argument('--url',
                        required=True,
                        action='store',
                        help='IP or FQDN for Vectra brain (http://www.example.com)')
